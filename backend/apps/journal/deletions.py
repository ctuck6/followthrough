...
from .models import Attachment, Execution, PendingFileDeletion, Rule, TradeReview, TradeStrategy


def ids_from(data, strings=False):
    ids = data.get("ids")
    if not isinstance(ids, list) or not ids or len(ids) > 10000:
        raise ValueError("Select one or more items.")
    if any(
        (not isinstance(x, str) or not x) if strings else (type(x) is not int or x < 1) for x in ids
    ):
...
def delete_rules(data):
    ...
    if rows.count() != len(ids):
        raise ValueError("Some rules no longer exist. Reload and try again.")
    rows.update(active=False)
    return {"rules": list(Rule.objects.filter(active=True).values("id", "text", "weight"))}


def delete_executions(data, account_id="all"):
    ...
    with transaction.atomic():
        ...
        if rows.count() != len(ids):
            raise ValueError("Some executions no longer exist. Reload and try again.")
        before = ledger(account_id)["trades"]
        rows.delete()
        result = ledger(account_id)
        for old in before:
            if not ids.intersection(old["execution_ids"]):
                continue
            survivors = set(old["execution_ids"]) - ids
            candidates = [t for t in result["trades"] if survivors.intersection(t["execution_ids"])]
            new = (
                max(candidates, key=lambda t: len(survivors.intersection(t["execution_ids"])))
                if candidates
                else None
            )
            key = old["trade_id"]
            if new and new["trade_id"] == key:
                continue
            ...
            if new:
                ...
                if review:
                    ...
                    dest.notes = "\n\n".join(filter(None, [dest.notes, review.notes]))
                    dest.save()
                    review.delete()
