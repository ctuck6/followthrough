...
export function CandleChart({data,storageKey}) {
 ...
 useEffect(()=>{
  ...
  const theme=()=>{const dark=document.documentElement.dataset.theme==='dark';const up='#a8d880',down='#ef9696';series.applyOptions({upColor:up,downColor:down,wickUpColor:up,wickDownColor:down});markerSeries.setMarkers(executionMarkers(data.candles,data.markers).map(marker=>({...marker,color:marker.shape==='arrowUp'?up:down})));const styles=getComputedStyle(container.current);chart.applyOptions({layout:{background:{type:ColorType.Solid,color:'#101b16'},textColor:'#dce9df'},grid:{horzLines:{color:'#2d4035'}},rightPriceScale:{borderColor:'#2d4035'},timeScale:{borderColor:'#2d4035',tickMarkFormatter:time=>format(time).replace(/:\d{2} /,' ')}})};
