...
export default function TradeDetails({trade,executions=[],onUpdated,request,csrfToken,notify,onClose}){
 ...
 const strategyRef=useRef(null),[strategySaving,setStrategySaving]=useState(false);
 ...
 return <Modal hideTitle title={`${trade.symbol} · ${trade.direction}`} className="trade-details-modal" busy={saving||strategySaving} onClose={async()=>{if(await saveAll())onClose()}}>
 <div className="trade-detail-banner"><span className="ticker-pill">{trade.symbol}</span><span>{trade.asset_class==='OPT'?`${trade.expiry} · ${Number(trade.strike)} ${trade.put_call==='C'?'Call':'Put'}`:trade.asset_class==='FUT'?'Future':'Stock'}</span>{trade.is_open&&<span className="trade-status">Open</span>}</div>
 ...
 <div role="tabpanel" className="trade-chart-layout" id="trade-panel-Chart" aria-labelledby="trade-tab-Chart" hidden={activeTab!=='Chart'}><aside tabIndex={0} className="trade-stats-sidebar" aria-label="Trade statistics"><h3>Stats</h3><dl className="trade-detail-grid">{[['Opened',trade.order_time],['Closed',trade.close_time||'Still open'],['Side',trade.direction],['Quantity',`${Number(trade.quantity)} ${trade.asset_class!=='STK'?'contracts':'shares'}`],['Remaining',Number(trade.remaining)],['Average entry',money(trade.price,trade.currency)],['Average exit',money(trade.exit_price,trade.currency)],['Commissions',money(trade.commission,trade.currency)],['Account',trade.account_alias||trade.account],['Currency',trade.currency],...(trade.asset_class!=='STK'?[['Multiplier',Number(trade.multiplier)]]:[])].map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}{['net','gross'].map(key=><div key={key}><dt>{key==='net'?'Net P&L':'Gross P&L'}</dt><dd className={`execution-pnl ${!trade.is_open&&trade[key]!==null?(Number(trade[key])<0?'is-loss':'is-gain'):''}`}>{pnl(key)}</dd></div>)}</dl></aside>
 <div className="trade-chart-main"><React.Suspense fallback={<p role="status">Loading chart…</p>}>{trade.asset_class==='FUT'?<p>Futures chart data is not available.</p>:<TradeChart key={trade.trade_id} trade={trade}/>}</React.Suspense></div></div>
 <div role="tabpanel" id="trade-panel-Strategy" aria-labelledby="trade-tab-Strategy" hidden={activeTab!=='Strategy'}><TradeStrategy ref={strategyRef} trade={trade} request={request} onBusyChange={setStrategySaving}/></div>
 ...
 <div role="tabpanel" id="trade-panel-Executions" aria-labelledby="trade-tab-Executions" hidden={activeTab!=='Executions'}><ExecutionList executions={executions} request={request} onUpdated={onUpdated} notify={notify} beforeDelete={saveAll}/></div>
 </div>
 </Modal>
}
