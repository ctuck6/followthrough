...
export default function TradeDetails({trade,request,csrfToken,notify,onClose}){
 ...
 return <Modal title={`${trade.symbol} · ${trade.direction}`} className="trade-details-modal" busy={saving} onClose={async()=>{if(await save())onClose()}}>
 <div className="trade-detail-banner"><span className="ticker-pill">{trade.symbol}</span><span>{trade.asset_class==='OPT'?`${trade.expiry} · ${Number(trade.strike)} ${trade.put_call==='C'?'Call':'Put'}`:'Stock'}</span><span className="trade-status">{trade.status}</span></div>
