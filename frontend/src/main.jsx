...
import {palettes,tokenNames} from './themes.js';
...
function Workspace({tab,setTab,selectedDate,setSelectedDate}){
 ...
 const [palette,setPalette]=useState(()=>{try{const saved=localStorage.getItem('followthrough-palette');return palettes[saved]?saved:'Grove'}catch{return 'Grove'}});
 useEffect(()=>{const root=document.documentElement;root.dataset.theme=theme;root.dataset.palette=palette;palettes[palette][theme].forEach((value,index)=>root.style.setProperty(`--${tokenNames[index]}`,value));try{localStorage.setItem('followthrough-theme',theme);localStorage.setItem('followthrough-palette',palette)}catch{}},[theme,palette]);
