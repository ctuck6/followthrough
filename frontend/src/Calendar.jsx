...
export default function Calendar({days, summaries={}, today, onOpen}) {
  ...
  return <div className="calendar-view">
    <section className="panel calendar-summary" aria-label="Average execution grade">
      ...
      <div className="range-controls"><span className="field-label">Average timeframe</span><button type="button" className="date-picker-trigger" aria-haspopup="dialog" aria-label="Choose average timeframe" onClick={()=>setPickerOpen(true)}>{period==='custom'?'Custom range':rangePresets.find(([key])=>key===period)?.[1]} <span aria-hidden="true">⌄</span></button><small>{start} → {end}</small></div>
